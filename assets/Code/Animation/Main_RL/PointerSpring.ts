  ...
  playPointerSwing3(totalTime: number, reboundTime: number) {
    ...
    swingIntervals.forEach((dt, idx) => {
      ...
      if (idx == 0) {
        ...
        // === 倒數第4下
        seq = seq
          .to(half, { angle: 40 }, { easing: 'sineOut' }) // 上擺
          .call(() => {
        ...
        seq = seq
          ...
          .to(half * 1.2, { angle: 15 }, { easing: 'sineIn' }) // 被彈回
          .call(() => {
            this.Audio.AudioSources[4].play();
        ...
        // 倒數第2下
        seq = seq
          .to(half * 0.8, { angle: 37 }, { easing: 'sineOut' }) // 上擺
          .delay(0.2)
          .call(() => {
            console.log(`🔼 倒數第2下 上擺結束: ${this.pivotNode.angle.toFixed(2)}°`);
          })
          .to(half * 1.2, { angle: 25 }, { easing: 'sineIn' }) // 被彈回
          .call(() => {
            this.Audio.AudioSources[4].play();
        ...
        // === 倒數第1下：對應盤面「超轉 → 彈回」===
        seq = seq
          .to(half * 0.3, { angle: 29 }, { easing: 'sineOut' }) // 卡住
          .delay(0.5)
          .call(() => {
        ...
        seq = seq
          ...
          .to(half, { angle: 36 }, { easing: 'quartIn' })
          .call(() => {
            this.Audio.AudioSources[4].play();
  ...
  playPointerSwing2(totalTime: number, reboundTime: number, holdTime: number = 0.5) {
    ...
    swingIntervals.forEach((dt, idx) => {
      ...
      if (idx == 0) {
        ...
        seq = seq
          .delay(holdTime * 0.2)
          .to(half, { angle: swingAngle }, { easing: 'sineOut' }) // 20
          .call(() => {
            // this.Audio.AudioSources[4].play();
            // console.log(`第 ${idx + 1} 下 🔼 上擺結束(最後一下): ${this.pivotNode.angle.toFixed(2)}°`);
          })
          .to(half * 1.7, { angle: 0 }, { easing: 'quartIn' })
          .call(() => {
            // this.Audio.AudioSources[4].play();
  ...
  playPointerSwing(totalTime: number, reboundTime: number, holdTime: number = 0.5) {
    ...
    swingIntervals.forEach((dt, idx) => {
      ...
      if (idx == 0) {
        ...
        // ✅ 倒數最後一下：上擺後停住
        seq = seq
          .to(half, { angle: swingAngle - 3 }, { easing: 'sineOut' })
          .delay(holdTime * 1.25) // 在上擺位置停
          // .call(() => this.Audio.AudioSources[4].play())
